...
<head>
    <meta charset="UTF-8">
    <title>{{ __('system.pages.print_service') }}</title>
    <style>
...
<body>
    <h1>{{ __('system.pages.print_service') }}</h1>
        ...
        <tbody>
            @forelse ($bookings as $index => $booking)
                <tr>
                    ...
                    <td class="text-right">{{ number_format((float) ($booking['service_fee'] ?? 0), 2) }}</td>
                </tr>
            @empty
        ...
        @if (!empty($bookings))
            <tfoot>
                <tr>
                    ...
                    <td class="text-right">{{ number_format(collect($bookings)->sum(fn ($row) => (float) ($row['service_fee'] ?? 0)), 2) }}</td>
                </tr>
            </tfoot>
        @endif
    </table>
